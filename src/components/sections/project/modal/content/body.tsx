    ...
    <div className="flex justify-between px-6 w-full max-lg:flex-col gap-x-14">
      {
        activeItem.details.body.map((item, index) => (
          <div key={`body-${index}`} className="flex flex-col basis-1/2 py-20 max-lg:py-16 max-md:py-12">
            ...
            {
              typeof item.text === 'string' ? <p className="font-pretendard text-2xl max-md:text-lg break-keep font-light">{item.text}</p> :
              (
                item.text.map((str, index) => (
                  <p key={`modal-nested-paragraph-${index}`} className={`font-pretendard text-2xl max-md:text-lg break-keep font-light ${index !== item.text.length - 1 ? 'mb-2' : ''}`}>{str}</p>
                ))
              )
            }
          </div>
